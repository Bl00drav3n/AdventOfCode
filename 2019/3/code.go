...
func parse(str string) []Segment {
	...
	for _, s := range ins {
		...
		case "D":
			y2 -= len
		default:
			panic(errors.New("Invalid direction: " + dir))
		}
		result = append(result, Segment{Point{x1, y1}, Point{x2, y2}})
		x1 = x2
		y1 = y2
	}
	return result
...
func isInSegment1D(x, a, b int) bool {
	if a > b {
		tmp := a
		a = b
		b = tmp
...
func areIntersecting(hor, ver Segment) bool {
	return isInSegment1D(ver.a.x, hor.a.x, hor.b.x) && isInSegment1D(hor.a.y, ver.a.y, ver.b.y)
}

func checkIntersection(seg1, seg2 Segment) (bool, Point) {
	var hor, ver Segment
	var result bool
	var p Point
	...
	if areIntersecting(hor, ver) {
		p = Point{ver.a.x, hor.a.y}
		result = true
	}
	return result, p
}

func findClosestIntersection(path1, path2 []Segment) int {
	closest := SINT32_MAX
	for _, seg1 := range path1 {
		for _, seg2 := range path2 {
			...
			intersecting, s := checkIntersection(hor, ver)
			if intersecting {
				closest = Min(closest, getDistance(s, Point{}))
			}
		}
	}
	return closest
...
func findShortestPath(path1, path2 []Segment) int {
	shortest := SINT32_MAX
	seg1dist := 0
	for _, seg1 := range path1 {
		seg2dist := 0
		for _, seg2 := range path2 {
			...
			intersecting, s := checkIntersection(hor, ver)
			if intersecting {
				d := seg1dist + getDistance(s, seg1.a) + seg2dist + getDistance(s, seg2.a)
				shortest = Min(shortest, d)
			}
			seg2dist += getDistance(seg2.a, seg2.b)
		}
		seg1dist += getDistance(seg1.a, seg1.b)
	}
	return shortest
}

func test1(wire1, wire2 string, d int) {
	if findClosestIntersection(parse(wire1), parse(wire2)) != d {
		panic("Failed test")
	} else {
		fmt.Println("Test ok")
	}
...
func test2(wire1, wire2 string, d int) {
	if findShortestPath(parse(wire1), parse(wire2)) != d {
		panic("Failed test")
	} else {
		fmt.Println("Test ok")
	}
