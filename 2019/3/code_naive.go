...
func findIntersections(a1, a2 []Point) []Point {
	var intersections []Point
	for _, elem1 := range a1 {
		for _, elem2 := range a2 {
			if elem1 == elem2 {
				intersections = append(intersections, elem1)
			}
		}
	}
...
func findClosestIntersectionDistance(a1, a2 []Point) int {
	intersections := findIntersections(a1, a2)
	closest := SINT32_MAX
	for _, elem := range intersections {
		closest = Min(closest, getDistance(elem, Point{0, 0}))
	}
	return closest
}

func getPathLength(a []Point, p Point) int {
	for j, q := range a {
		if p == q {
			return j + 1
		}
...
func findShortestPath(a1, a2 []Point) int {
	intersections := findIntersections(a1, a2)
	lengths1 := make([]int, len(intersections))
	lengths2 := make([]int, len(intersections))
	for i, p := range intersections {
		lengths1[i] = getPathLength(a1, p)
		lengths2[i] = getPathLength(a2, p)
	}
	shortest := SINT32_MAX
	for i := range intersections {
		shortest = Min(shortest, lengths1[i]+lengths2[i])
	}
	return shortest
}

func test1(wire1, wire2 string, d int) {
	if findClosestIntersectionDistance(parse(wire1), parse(wire2)) != d {
		panic("Failed test")
	} else {
		fmt.Println("Test ok")
	}
...
func part1(path1, path2 []Point) {
	start := time.Now()
	fmt.Printf("Closest intersection: %d\n", findClosestIntersectionDistance(path1, path2))
	fmt.Printf("Time: %s\n", time.Since(start))
}

func part2(path1, path2 []Point) {
	start := time.Now()
