...
def min_dist(dist, Q):
    d = math.inf
    p = None
    # TODO(rav3n): This is slow. At some point we might want to learn how to use heapq 
    # for using it as a min-priority queue
...
def dijkstra(grid):
    ...
    Q = set()
    [[Q.add((i,j)) for i in range(len(grid[j]))] for j in range(len(grid))]
    width = len(grid[0])
    height = len(grid)
    while Q:
